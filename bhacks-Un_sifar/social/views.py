...
def show(request):
	...
	k=obj.subscribed.all()
	for pie in pro:
		sub=False
		if pie in k:
			sub=True
		pink={'teacher':pie.teacher,'name':pie.name,'price':pie.price,'logo':pie.logo,'is_completed':pie.is_completed,
		'description':pie.description,'id':pie.id,'subscribe':sub}
		god.append(pink)
	print(obj.subscribed.all())
	l=0
	context={'god':god,'hi':obj}
	return render(request,'social/courses.html',context)
def lecture(request,pk,user_id=1):
	obj=newuser.objects.filter(user=request.user).get()
	pro=course.objects.filter(pk=pk)
	y=lectures.objects.filter(course__exact=pro[0])
	by=lectures.objects.filter(pk=user_id).get()
	con=[]
	for t in y:
		pink={'video':t.video,'name':t.name,'mentor':t.course.user.username,'id':t.id}
		con.append(pink)
	context={'computer':con,'ki':pk,'user_id':by.id,'by':by,'hi':obj}
	print(con)
	return render(request,'social/lecture.html',context)
@login_required
def subscribe(request,pk):
	if request.user.is_authenticated:
		cup=course.objects.filter(pk=pk).get()
		user=request.user
...
def add_lecture(request,pk):
	if request.method=='POST':
		...
		return render(request,'social/add_lecture.html',context)
